defmodule YCWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: YCWeb.Gettext
  ...
  @doc """
  Renders an auth button to use for social logins (i.e., Facebook).

  ## Examples

      <.auth_button social={:facebook} method={:register}/>
  """
  attr :social, :string, values: ~w(facebook), default: "facebook"
  attr :method, :string, values: ~w(login register), default: "login"

  def auth_button(%{social: "facebook"} = assigns) do
    ~H"""
    <.link
      href="/auth/facebook"
  ...
  def input(%{type: "hidden"} = assigns) do
    ~H"""
    <input type="hidden" id={@id} value={@value} name={@name} value="false" />
    """
  end
  ...
  def picture_preview(assigns) do
    ...
    <div class="relative" class="w-[200px] h-[200px]">
      ...
      <img
        ...
        alt="Picture Preview"
        class="my-2 rounded-full border-4 w-[200px] h-[200px] object-cover"
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(YCWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(YCWeb.Gettext, "errors", msg, opts)
    end
  end
