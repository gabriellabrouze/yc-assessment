defmodule YC.Accounts do
  ...
  alias YC.Accounts.{User, UserToken, UserNotifier}
  ...
  @doc """
  Retrieves user with email if user exists, else registers
  a new user with a random password.
  """
  def get_user_by_email_or_register(user) when is_map(user) do
    case Repo.get_by(User, email: user["email"]) do
      nil ->
        # no user exists, register user
        # user needs some password -> generate it but don't notify the user
        pw = :crypto.strong_rand_bytes(30) |> Base.encode64(padding: false)

        {:ok, user} =
          Map.put(user, "password", pw)
          |> register_user()
